...
class TestMemoryRetriever(BaseTestComponent):
    ...
    def test_to_dict_with_store_instance(self):
        retriever = MemoryRetriever()
        retriever.store = MemoryDocumentStore()
        data = retriever.to_dict()
        assert data == {
            "hash": id(retriever),
            "type": "MemoryRetriever",
            "store": {
                "hash": id(retriever.store),
    ...
    @pytest.mark.unit
    def test_to_dict_with_store_name(self):
        retriever = MemoryRetriever()
        retriever._store_name = "some_store"
        data = retriever.to_dict()
        assert data == {
            "hash": id(retriever),
            "type": "MemoryRetriever",
    ...
    def test_from_dict(self):
        ...
        assert retriever._store == None
        assert retriever._store_name == ""
    ...
    def test_from_dict_with_store_instance(self):
        ...
        retriever = MemoryRetriever.from_dict(data)
        assert isinstance(retriever._store, MemoryDocumentStore)
        assert retriever._store_name == ""
        assert retriever.filters == None
        assert retriever.top_k == 10
        assert retriever.scale_score
    ...
    @pytest.mark.unit
    def test_from_dict_with_store_name(self):
        data = {
            "hash": 1234,
            "type": "MemoryRetriever",
            "store": "some_store",
